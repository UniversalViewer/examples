...
const overlayAnimation = async (overlay, animationBuilder, baseEl, opts) => {
    ...
    const animation = await new Promise(function (resolve) { resolve(require('./index-68424d7d.js')); }).then(mod => mod.create(animationBuilder, aniRoot, opts));
...
const safeCall = (handler, arg) => {
    if (typeof handler === 'function') {
        const jmp = __chunk_2.config.get('_zoneGate', defaultGate);
        return jmp(() => {
            try {
                return handler(arg);
            }
