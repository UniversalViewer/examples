...
if (!win.__stencil_cssshim && needsShim()) {
    ...
    var regex = /\/uv-ebook-components(\.esm)?\.js($|\?|#)/;
    var scriptElm = currentScript || Array.from(doc.querySelectorAll('script')).find(function(s) {
      return regex.test(s.src) || s.getAttribute('data-stencil-namespace') === "uv-ebook-components";
    });

    var resourcesUrl = scriptElm ? scriptElm.getAttribute('data-resources-url') || scriptElm.src : '';
    var start = function() {
      var url = new URL('./p-99d1d851.system.js', resourcesUrl);
