...
const setRootPath = function setRootPath(path) {
  rootPath = path;
};
/**
...
const setSanitizeDOMValue = function setSanitizeDOMValue(newSanitizeDOMValue) {
...
const setPassiveTouchGestures = function setPassiveTouchGestures(usePassive) {
...
const setStrictTemplatePolicy = function setStrictTemplatePolicy(useStrictPolicy) {
...
const setAllowTemplateFromDomModule = function setAllowTemplateFromDomModule(allowDomModule) {
...
const dedupingMixin = function dedupingMixin(mixin) {
...
function importModule(moduleId) {
  return (
    /** @type {?DomModule} */
    dom_module_DomModule.import(moduleId)
  );
}
...
function runNotifyEffects(inst, notifyProps, props, oldProps, hasPaths) {
  ...
  let notified;
  let id = property_effects_dedupeId++; // Try normal notify effects; if none, fall back to try path notification
...
function parseArg(rawArg) {
  ...
  if (!a.literal) {
    a.rootProperty = path_root(arg); // detect structured path (has dots)
...
const property_effects_PropertyEffects = dedupingMixin(superClass => {
  ...
  class PropertyEffects extends propertyEffectsBase {
    ...
    _setPendingPropertyOrPath(path, value, shouldNotify, isPathNotification) {
      if (isPathNotification || path_root(Array.isArray(path) ? path[0] : path) !== path) {
...
const ElementMixin = dedupingMixin(base => {
  ...
  function getTemplateFromDomModule(is) {
    ...
    if (is && (!strictTemplatePolicy || allowTemplateFromDomModule)) {
      template = dom_module_DomModule.import(is, 'template'); // Under strictTemplatePolicy, require any element with an `is`
    ...
    static get importPath() {
      if (!this.hasOwnProperty(JSCompiler_renameProperty('_importPath', this))) {
        ...
        if (meta) {
          ...
          const module = dom_module_DomModule.import(
          /** @type {PolymerElementConstructor} */
          this.is);
...
const updateStyles = function updateStyles(props) {
  if (window.ShadyCSS) {
...
const htmlLiteral = function htmlLiteral(strings) {
...
class my_el_MyEl extends polymer_element_PolymerElement {
  static get template() {
    return html`
      <style>
        :host {
...
window.customElements.define('my-el', my_el_MyEl);
