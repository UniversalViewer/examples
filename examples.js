...
$(function(){
    ...
    function openEditor() {
        ...
        if (sessionConfig){
            ...
            $.getJSON('/build/uv-1.1.3/js/' + configName + '.config.js', function(config){
                $('.config-name').text('(' + configDisplayName + ')');
                showEditor();
                editor.setValue(config);
            });
    ...
    function init() {
        if (isLocalhost){
            ...
            $("body").append('<script type="text/javascript" id="embedUV" src="/build/uv-1.1.3/js/embed.js"><\/script>');
        }
